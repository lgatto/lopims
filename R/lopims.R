...
##' \code{\link[synapter]{synapter}} package and combines resulting
##' \code{\link[synapter]{Synapter}} instances into one
##' \code{"\linkS4class{MSnSet}"} and organelle marker data is added
##' as a feature-level annotation variable.
##'
...
##' reported (see lopims:::refNormMeanOfNonNAPepSum for
...
##'     containing the HDMSe final peptide files. Default is
##'     \code{HDMSe}.
##'
...
##'
##' @param fastafile A \code{character} identifying the protein fasta
##'     database. Default is to use the fasta file in the current
##'     directory. If several such files exist, the function reports
##'     an error.
##'
##' @param markerfile A \code{character} identifying the marker file
##'     (see details for format). Default is to use a \code{csv} file
##'     starting with \code{marker} in the current directory.  If
##'     several such files exist, the function reports an error.
##'
...
##'     protein level quantitation and respective organelle markers.
##'
